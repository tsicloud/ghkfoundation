(function () {
  ...
  async function generateQR() {
    ...
    // Fallback to davidshimjs qrcode.js API
    if (isShimAPI()) {
      setStatus('Generating…');
      // Clear wrapper and render inside it.
      if (previewWrap) previewWrap.innerHTML = '';
      const holder = document.createElement('div');
      if (previewWrap) previewWrap.appendChild(holder);
      try {
        // qrcode.js renders <canvas> or <img> inside holder
        /* global QRCode */
        ...
        setStatus('Done. (SVG download not available with this local library)');
        enableDownloads(true, false); // PNG only
      } catch (e) {
        console.error(e);
  ...
  function downloadPng() {
    // Try canvas from soldair API first (our fixed <canvas>)
    let cnv = canvas;
    // Or canvas generated by qrcodejs inside previewWrap
  ...
  async function downloadSvg() {
    if (!isSoldairAPI()) {
      setStatus('SVG download not supported with this local library.');
      return;
    }
    const { text, size, margin, ecc, dark, light } = getOpts();
    try {
